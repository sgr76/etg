    ...
    if (out.is_open())
    {
        ...
        for (auto & D : defs)
        {
            ...
            if (D.second->getFormat())
            {
                ...
                {
                    out << "    case 'd':" << std::endl;
                    out << "      return format_to(ctx.out(), \"{}\", etg<" << fqEnum << ">::debugSymbol(d).toStdString());" << std::endl;
                }

                if (D.second->getUseToken())
                {
                    out << "    case 't':" << std::endl;
                    out << "      return format_to(ctx.out(), \"{}\", etg<" << fqEnum << ">::token(d).toStdString());" << std::endl;
                }

                if (D.second->getTranslate())
                {
                    out << "    case 'l':" << std::endl;
                    out << "      return format_to(ctx.out(), \"{}\", etg<" << fqEnum << ">::translation(d).toStdString());" << std::endl;
                }
